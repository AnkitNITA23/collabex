import { TextOperation, transform, compose } from './ot';
...
function runTests() {
  ...
  // Test Case 1: Simple insert convergence (same position insert)
  {
    ...
    const opA = new TextOperation().insert('A').retain(5);
    // opB: insert 'B' at start -> 'Bhello'
    const opB = new TextOperation().insert('B').retain(5);

    const [opAPrime, opBPrime] = transform(opA, opB, 'left');
  ...
  // Test Case 2: Concurrent non-overlapping deletes
  {
    ...
    const opA = new TextOperation().retain(1).delete(1).retain(4);
    // opB: delete 'e' (index 4) -> 'abcdf'
    const opB = new TextOperation().retain(4).delete(1).retain(1);
  ...
  // Test Case 3: Overlapping retain vs delete
  {
    ...
    const opA = new TextOperation().retain(1).delete(2).retain(3);
    // opB: retain 6 (no changes) -> 'abcdef'
    const opB = new TextOperation().retain(6);
  ...
  // Test Case 4: Operation composition
  {
    ...
    const op1 = new TextOperation().insert('world ').retain(5);
    // op2: retain 6 ('world '), delete 5 ('hello') -> 'world '
    const op2 = new TextOperation().retain(6).delete(5);

    const composedOp = compose(op1, op2);
